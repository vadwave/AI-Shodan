...
public class Guard : MonoBehaviour, IDamageable, IAttacking, IMovable, IPatroling, IEye
{
    ...
    [SerializeField] float distanceAttack = 7f;
    ...
    void InitializeExtensibleStateMachine()
    {
        ...
        At(allert, attack, DistanceAttack());

        At(attack, search, NotTargetVisible());
        At(attack, allert, NotDistanceAttack());
        ...
        Func<bool> DistanceAttack() => () => distanceAttack > GetDistanceTarget();
        Func<bool> NotDistanceAttack() => () => distanceAttack < GetDistanceTarget();
    }
    ...
    {
        return Vector2.Distance(this.transform.position, GetTarget().position);
    }
